...
module mod_bound
  ...
  !
  private
  public  :: bounduvw, boundp, boundp_e, updt_rhs_b, boundp_c

public boundsb_array
  ...
  subroutine boundsb_array(n,s_bound,nh_p,halo,s)
    !   
    ...
    !   
    integer , intent(in   ), dimension(3)           :: n 
    real(rp), intent(in   ), dimension(1:,1:)       :: s_bound
    integer , intent(in   )                                     :: nh_p
    real(rp), intent(inout), dimension(1-nh_p:,1-nh_p:,1-nh_p:) :: s
    integer , intent(in   ), dimension(3)                       :: halo
    ...
    call updthalo(n(1),n(2),n(3),nh_p,halo(ind1),ind1,s)
    call updthalo(n(1),n(2),n(3),nh_p,halo(ind2),ind2,s)


    do q=0,nh_p-1
      do j=1,n(2)
        do i=1,n(1)
          s(i,j,0-q) = s_bound(i,j)
        enddo
      enddo
    enddo  
    !   
    return
  end subroutine boundsb_array
  ...
  subroutine updthalo(nx,ny,nz,nh,halo,idir,p)
    ...
    !
subroutine boundp_e(n,nh_d,nh_p,halo,dl,dzc,dzf,p,cp,h0,tmp)
    !
    use mod_common_mpi, only: MPI_PROC_NULL,left,right,front,back,top,bottom
    !
    !
    ...
    real(rp)        , intent(in   ), dimension(1-nh_d:)                 :: dzc,dzf
    real(rp)        , intent(inout), dimension(1-nh_p:,1-nh_p:,1-nh_p:) :: p
    real(rp)        , intent(in), dimension(1-nh_p:,1-nh_p:,1-nh_p:)    :: cp,h0,tmp
    !
    !
    integer :: i, j, k
    real(rp), dimension(0:nh_p-1) :: dr
    ...
    !
    !
    ! cp stands for rho*cp
    !
    ! along x
    !
    if(left .eq.MPI_PROC_NULL) then ! x - bottom part
    !
    do k=1-nh_p,n(3)+nh_p
       do j=1-nh_p,n(2)+nh_p
                  p(0,j,k  )   = cp(0,j,k)*tmp(0,j,k)+h0(0,j,k)
       enddo
    enddo
    !
    endif
    ...
    do k=1-nh_p,n(3)+nh_p
       do j=1-nh_p,n(2)+nh_p
                  p(n(1)+1,j,k) = cp(n(1)+1,j,k)*tmp(n(1)+1,j,k)+h0(n(1)+1,j,k)
       enddo
    enddo
    ! 
    endif
    ...
    !
    do k=1-nh_p,n(3)+nh_p
       do i=1-nh_p,n(1)+nh_p
                  p(i,0,k  ) = cp(i,0,k)*tmp(i,0,k)+h0(i,0,k)
        enddo
     enddo
    !
     endif
    ...
    !                                                                                                          
      do k=1-nh_p,n(3)+nh_p
        do i=1-nh_p,n(1)+nh_p
                  p(i,n(2)+1,k) = cp(i,n(2)+1,k)*tmp(i,n(2)+1,k)+h0(i,n(2)+1,k)
        enddo
      enddo
    ! 
    ...
    !        
      do j=1-nh_p,n(2)+nh_p
        do i=1-nh_p,n(1)+nh_p
                  p(i,j,0  ) = cp(i,j,0)*tmp(i,j,0)+h0(i,j,0)
    ...
    !        
      do j=1-nh_p,n(2)+nh_p
        do i=1-nh_p,n(1)+nh_p
                  p(i,j,n(3)+1) = cp(i,j,n(3)+1)*tmp(i,j,n(3)+1)+h0(i,j,n(3)+1)                                                  
       enddo                                                                                                  
     enddo
    ! 
    endif
    !
    return
  end subroutine boundp_e
  !
subroutine boundp_c(n,nh_d,nh_p,halo,dl,dzc,dzf,phi,rho,omega,c)
    !
    ...
    real(rp)        , intent(in   ), dimension(3)                       :: dl
    real(rp)        , intent(in   ), dimension(1-nh_d:)                 :: dzc,dzf
    real(rp)        , intent(inout), dimension(1-nh_p:,1-nh_p:,1-nh_p:) :: c
    real(rp)        , intent(in   ), dimension(1-nh_p:,1-nh_p:,1-nh_p:) :: phi,omega
    !
    !
    integer                       :: i, j, k
    real(rp), dimension(0:nh_p-1) :: dr
...
#endif
    !
    call updthalo(n(1),n(2),n(3),nh_p,halo(ind1),ind1,c)
    call updthalo(n(1),n(2),n(3),nh_p,halo(ind2),ind2,c)
    !
    ! along x
    !
    if(left .eq.MPI_PROC_NULL) then ! x - bottom part
    ...
    do k=1-nh_p,n(3)+nh_p
       do j=1-nh_p,n(2)+nh_p
                  c(0,j,k  )   = omega(0,j,k)*rho*(1.d0-phi(0,j,k))
       enddo
    enddo
    !
    endif
    ...
    do k=1-nh_p,n(3)+nh_p
       do j=1-nh_p,n(2)+nh_p
                  c(n(1)+1,j,k) = omega(n(1)+1,j,k)*rho*(1.d0-phi(n(1)+1,j,k)) 
       enddo
    enddo
    ! 
    endif
    ...
    do k=1-nh_p,n(3)+nh_p
       do i=1-nh_p,n(1)+nh_p
                  c(i,0,k  ) = omega(i,0,k)*rho*(1.d0-phi(i,0,k)) 
        enddo
     enddo
    !
     endif
    ...
    !                                                                                                          
      do k=1-nh_p,n(3)+nh_p
        do i=1-nh_p,n(1)+nh_p
                  c(i,n(2)+1,k) = omega(i,n(2)+1,k)*rho*(1.d0-phi(i,n(2)+1,k)) 
        enddo
      enddo
    ! 
    ...
    !        
      do j=1-nh_p,n(2)+nh_p
        do i=1-nh_p,n(1)+nh_p
                  ...
                  c(i,j,0  ) = omega(i,j,0)*rho*(1.d0-phi(i,j,0)) 
    ...
    !        
      do j=1-nh_p,n(2)+nh_p
        do i=1-nh_p,n(1)+nh_p
                  ...
                  c(i,j,n(3)+1) = omega(i,j,n(3)+1)*rho*(1.d0-phi(i,j,n(3)+1)) 
