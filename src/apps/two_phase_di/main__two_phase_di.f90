...
program flutas
  ...
  use mod_di_acdi   , only: rk_psi,initls,initpsi,psi_to_ls,cmpt_norm,ls_to_psi,cmpt_umax,update_property
  ...
  real(rp) :: psi_min,psi_max,vol_p1
  real(rp) :: aux
  integer  :: ii,jj
  integer :: n1, n2, n3
  integer :: ng1, ng2, ng3
  ...
  if(.not.restart) then
    ...
    if(.not.late_init) then 
      ...
      ! Initialize DI 
      !
      !call initls('tgn',n,dl,l,nh_p,ls)
      !call boundp(cbcpsi,n,bcpsi,nh_d,nh_v,halo_v,dl,dzc,dzf,ls )
      !call ls_to_psi(n,dl,nh_p,ls,eps_int,psi)
  ...
  var(5) = psi_max
  var(6) = vol_p1
  call out0d(trim(datadir)//'psi_info.out',6,var)
  !
  ...
  do while(.not.is_done)
    ...
    call rk_psi(f_t1,f_t2,n,dli,nh_d,nh_u,nh_v, &
                dzc,dzf,u_reg,eps_int,u,v,w,psi,ls,nor,dpsidtrko)
    call boundp(cbcpsi,n,bcpsi,nh_d,nh_v,halo_v,dl,dzc,dzf,psi)
    !
    call psi_to_ls(n,dl,nh_p,psi,eps_int,ls)
    call boundp(cbcpsi,n,bcpsi,nh_d,nh_v,halo_v,dl,dzc,dzf,ls)
