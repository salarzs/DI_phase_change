...
program flutas
  ...
  use mod_param     , only: lx,ly,lz,dx,dy,dz,dxi,dyi,dzi,small,is_wallturb, &
                            ...
                            restart_mph_heat_scalar,constant_dt,dt_input, &
                            inivel,inivap, &
                            ...
                            rho0,cbctmp,bctmp,tl0,tg0,cp1,cp2,cond1,cond2, &
                            h01,h02,t_relax_i,pth0,    &
  ...
  real(rp), dimension(:,:,:), allocatable :: mu,rho,muold,rhoold,rho_new,rho_gas
  ...
  !
  real(rp), dimension(:,:,:)  , allocatable :: psi,kappa,psiold,kappaold 
  real(rp), dimension(:,:,:,:), allocatable :: nor,norold                   
  real(rp), dimension(:,:,:)  , allocatable :: ls,dpsidtrko
  real(rp), dimension(:,:,:)  , allocatable :: c1,c2,dc1dtrko,dc2dtrko,src_ph,omega_s,diff,omega,p_sat
  real(rp), dimension(:,:,:)  , allocatable :: omega_diff,omega_diff_ex
  real(rp), dimension(:,:,:)  , allocatable :: ent,rhoh,tmp,tmp_int,cpp,cond,rhocp,dentdtrko
  real(rp), dimension(:,:,:)  , allocatable :: cppold,condold,rhocpold,lh
...
real(rp), dimension(:,:)  , allocatable :: psi_bound,c1_bound
  ...
  integer  :: nh_d,nh_u,nh_p,nh_v,ps_step
  !
  ...
  real(rp) :: rho0i,u_reg,tmp_min,tmp_max,tmp_int_c,ind
  ...
  real(rp) :: psi_min,psi_max,vol_p1,vol_c1,vol_c2,vol_ct,sr_t,omega_max,omega_min,omega_t
  real(rp) :: m_a, m_v,h1,h2
  real(rp) :: pth,t_relax,t_rel
  ...
  allocate(c1(0:n(1)+1,0:n(2)+1,0:n(3)+1)        , &
           ...
           omega_s(0:n(1)+1,0:n(2)+1,0:n(3)+1)        , &
           omega_diff(0:n(1)+1,0:n(2)+1,0:n(3)+1)        , &
           omega_diff_ex(0:n(1)+1,0:n(2)+1,0:n(3)+1)        , &
           p_sat(0:n(1)+1,0:n(2)+1,0:n(3)+1)        , &
           ent(0:n(1)+1,0:n(2)+1,0:n(3)+1)        , &
           lh(0:n(1)+1,0:n(2)+1,0:n(3)+1)          ,&
           rhoh(0:n(1)+1,0:n(2)+1,0:n(3)+1)        , &
           tmp(0:n(1)+1,0:n(2)+1,0:n(3)+1)        , &
           tmp_int(0:n(1)+1,0:n(2)+1,0:n(3)+1)        , &
...
allocate(c1_bound(1:n(1),1:n(2))) 
  ...
  if(.not.restart) then
    ...
    !
    !Here we deal with initializing the rho*h0 mixture  
    !
    call update_property(n,(/h01*rho1,h02*rho2/),psi,lh)
    call boundp(cbcpsi,n,bcpsi,nh_d,nh_v,halo_v,dl,dzc,dzf,lh)
    !
    !Initialize temperature and bound it 
    !
    call update_property(n,(/tl0,tg0/),psi,tmp)
    call boundp(cbctmp,n,bctmp,nh_d,nh_v,halo_v,dl,dzc,dzf,tmp)
    !
    h1 = rho1*(cp1*tl0+h01)
    h2 = rho2*(cp2*tg0+h02)
    !
    call update_property(n,(/h1,h2/),psi,ent)
    call boundp_e(n,nh_d,nh_p,halo_v,dl,dzc,dzf,ent,rhocp,lh,tmp) !impose b.c on "ent"
    !
#else
    tmp(:,:,:) = tmp0
#endif
...
#if defined(_PHASE_CHANGE)
    pth = pth0
    call initvap(inivap,n,dli,nh_d,nh_u,nh_v,halo_v,dzc,dzf,diff2,rho2,pth,tmp,psi,nor,c2,omega,src_ph)
#else
...
if(restart_sph) then 
    ...
    call load_scalar(action_load,trim(restart_dir)//'scalar.out',time,t_relax,istep,dto)
    !
#if defined(_TWO_PHASE)
    call initpsi(inipsi,n,dl,l,nh_v,eps_int,psi)
#if defined(_CONTACT_LINE)
...
if(restart_mph_scalar) then 
    ...
    call load(action_load,trim(restart_dir)//'fldomg.bin',n,    omega(1:n(1),1:n(2),1:n(3)))
    ...
    call boundp_c(n,nh_d,nh_v,halo_v,dl,dzc,dzf,psi,rho2,omega,c2)
    do i=1,n(1)
     do j=1,n(2)
      do k=1,n(3)
    ...
    call cmpt_src(n,dli,nh_d,nh_u,nh_v,dzc,dzf,psi,rho2,t_relax,omega,omega_s,src_ph)
...
if(restart_mph_heat) then
    ...
    call load(action_load,trim(restart_dir)//'fldent.bin',n,    ent(1:n(1),1:n(2),1:n(3)))
    call load(action_load,trim(restart_dir)//'flddent.bin',n,dentdtrko(1:n(1),1:n(2),1:n(3)))
    call load_scalar(action_load,trim(restart_dir)//'scalar.out',time,t_relax,istep,dto)
    !
#if defined(_CONTACT_LINE)
    call static_contact_angle_m(n,dl,nh_v,eps_int,theta,psi,psi_bound)
    ...
    omega(:,:,:) = 0.d0
#endif
    if(myid.eq.0) print*, '*** multiphase-heat-transport Checkpoint loaded at time = ', time, 'time step = ', istep, '. ***'
endif
  !!!!
  if(restart_mph_heat_scalar) then
    action_load = 'r'
    call load(action_load,trim(restart_dir)//'fldu.bin',n,      u(1:n(1),1:n(2),1:n(3)))
    call load(action_load,trim(restart_dir)//'fldv.bin',n,      v(1:n(1),1:n(2),1:n(3)))
    call load(action_load,trim(restart_dir)//'fldw.bin',n,      w(1:n(1),1:n(2),1:n(3)))
    ...
    !
    !
    if(myid.eq.0) print*, '*** multiphase-heat-scalar-transport Checkpoint loaded at time = ', time, 'time step = ', istep, '. ***'
endif
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1!!!!!!!!!!!!
!
...
!
...
#endif
  ! 
  !Update mutual thermophysical properties
  !
...
#if defined(_TWO_PHASE)          
    call chkdt_tw(n(1),n(2),n(3),dxi,dyi,dzi,nh_d,nh_u,dzci,dzfi,u,v,w,dtmax,t_rel)
#else    
  ...
  ! 
...
#if defined(_PHASE_CHANGE)
      sr_t  = sum(src_ph(1:n(1),1:n(2),1:n(3)))*dl(1)*dl(2)*dl(3)
      call mpi_allreduce(MPI_IN_PLACE,sr_t,1,MPI_REAL_RP,MPI_SUM,MPI_COMM_WORLD,ierr)
#endif
      ...
      omega_t   = sum(omega(1:n(1),1:n(2),1:n(3)))*dl(1)*dl(2)*dl(3)
      call mpi_allreduce(MPI_IN_PLACE,omega_t,1,MPI_REAL_RP,MPI_SUM,MPI_COMM_WORLD,ierr)
#endif
      !
      var(:) = 0._rp
      var(1) = 1._rp*istep
      ...
      var(6) = vol_ct
      var(7) = omega_t           
      var(8) = omega_max               !max value of vapor mass fraction
      var(9) = omega_min               !min value of vapor mass fraction
      var(10) = sr_t
  ...
  do while(.not.is_done)
    ...
    !
    !Update properties at time-step (n)
    ! 
    call update_property(n,(/rho1 ,rho2 /),psi,rho)
    call boundp(cbcpsi,n,bcpsi,nh_d,nh_v,halo_v,dl,dzc,dzf,rho)
    !
    ...
    call rk_c1(f_t1,f_t2,n,dli,nh_d,nh_u,nh_v, &
               dzc,dzf,diff1,diff2,u_reg,eps_int,u,v,w,ur,vr,wr,c1,c2,src_ph,psi,nor,dc1dtrko,dc2dtrko,-1) 
    !
    call boundp_c(n,nh_d,nh_v,halo_v,dl,dzc,dzf,psi,rho2,omega,c2)                                                    !This could be modified 
    !
    !Only needed in case of very high spatial-resolution 
    !
    do i = 1,n(1)
      do j = 1,n(2)
        do k = 1,n(3)
         if (psi(i,j,k).ge.0.999999999999d0)then
             c2(i,j,k) = 0.d0
         endif
        enddo
       enddo
    ...
    call rk_c1(f_t1,f_t2,n,dli,nh_d,nh_u,nh_v, &
                dzc,dzf,diff1,diff2,u_reg,eps_int,u,v,w,ur,vr,wr,c1,c2,src_ph,psi,nor,dc1dtrko,dc2dtrko,+1)  !SCALAR TRANS. (Scalar inside the dispersed phase) 
    !    
    call boundp(cbcsca,n,bcsca,nh_d,nh_v,halo_v,dl,dzc,dzf,c1)
    ! 
    call rk_c1(f_t1,f_t2,n,dli,nh_d,nh_u,nh_v, &
                dzc,dzf,diff1,diff2,u_reg,eps_int,u,v,w,ur,vr,wr,c1,c2,src_ph,psi,nor,dc1dtrko,dc2dtrko,-1)!SCALAR TRANS. (Scalar outside the dispersed phase) 
    ! 
    call boundp(cbcsca,n,bcsca,nh_d,nh_v,halo_v,dl,dzc,dzf,c2)    
    !
    ! 
   ...
   !
   !
   !Update the order-parameter at time-step (n+1)
   !
  ...
  !
   ...
   !
   ! 
   !Update vapor mass at time-step (n+1)
   !
#if defined(_PHASE_CHANGE)
   do i = 1,n(1)
     do j = 1,n(2)
       do k = 1,n(3)
         omega(i,j,k) = c2(i,j,k)/(rho2*(1.0-psi(i,j,k))+1e-30) 
      enddo
     enddo
   enddo
   !
   ...
   !
    ...
    !
    !
    !Consistancy terms are not added for ACDI yet
    !
#if defined(_HEAT_TRANSFER)
    gradpx = 0.d0
    psifx = 0.d0
    ...
    do k=1,n(3)
     do j=1,n(2)
      ...
      !
      mfx(i,j,k)  = (rho1*(cp1*tmpx+h01)-rho2*(cp2*tmpx+h02))*regx
      mfy(i,j,k)  = (rho1*(cp1*tmpy+h01)-rho2*(cp2*tmpy+h02))*regy
      mfz(i,j,k)  = (rho1*(cp1*tmpz+h01)-rho2*(cp2*tmpz+h02))*regz
      !
      enddo
     enddo
    enddo
    ...
    !
    !Update total enthalpy and temperature  at time-step "n+1"
    !
    call rk_ent(f_t1,f_t2,n,dli,nh_d,nh_u,nh_v,dzc,dzf,eps_int,u_reg, &
               u,v,w,psiold,norold,tmp,condold,mfx,mfy,mfz,ent,dentdtrko)
    !
    call boundp_e(n,nh_d,nh_p,halo_v,dl,dzc,dzf,ent,rhocp,lh,tmp) ! b.c imposed on enthalpy
    !
     do i=1,n(1)
      do j=1,n(2)
       do k=1,n(3)
...
!
  ...
  enddo
...
#if defined(_PHASE_CHANGE)    
    !
    !Update the saturation vapor-mass and interfacial mass-flux at time-step n+1
    !
    pth = pth0
    !
    do i=1,n(1)
    ...
    !
    !
    call fillps(n(1),n(2),n(3),nh_d,nh_u,dxi,dyi,dzi,dzfi,f_t12_i,rho0,rho1,rho2,src_ph,u,v,w,p)
    !
    ...
    if(mod(istep,icheck).eq.0) then
      ...
      !if(divmax.gt..or.divtot.ne.divtot) then
        if(isnan(divmax).or.isnan(divtot)) then
        if(myid.eq.0) print*, 'ERROR: Code is Crashed due to divergence'
        if(myid.eq.0) print*, 'Aborting...'
        is_done = .true.
        kill = .true.
      endif
    ...
    if ( (mod(istep,isave) .eq. 0) .or. is_done .and. (.not.kill) ) then
      ...
      call load(action_load,trim(restart_dir)//'flddc2.bin',n, dc2dtrko(1:n(1),1:n(2),1:n(3)))
#if defined(_PHASE_CHANGE)
      inquire(file='data/fldomg.bin', exist=is_data)
      if(myid.eq.0.and.is_data) call execute_command_line('mv data/fldomg.bin data/fldomg_old.bin')
      call load(action_load,trim(restart_dir)//'fldomg.bin',n, omega(1:n(1),1:n(2),1:n(3)))
#endif   
#endif
#if defined(_HEAT_TRANSFER)
      ...
      inquire(file='data/fldent.bin', exist=is_data)
      if(myid.eq.0.and.is_data) call execute_command_line('mv data/fldent.bin data/fldent_old.bin')
      call load(action_load,trim(restart_dir)//'fldent.bin',n, ent(1:n(1),1:n(2),1:n(3)))

      inquire(file='data/flddent.bin', exist=is_data)
      if(myid.eq.0.and.is_data) call execute_command_line('mv data/flddent.bin data/flddent_old.bin')
      call load(action_load,trim(restart_dir)//'flddent.bin',n, dentdtrko(1:n(1),1:n(2),1:n(3)))
#endif
      !
#if defined(_PHASE_CHANGE)
      t_relax = t_relax*1.d0
#else
      t_relax = t_relax*0.d0
#endif
