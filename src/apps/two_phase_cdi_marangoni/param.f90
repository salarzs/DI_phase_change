...
module mod_param
  ...
  logical                              :: restart_sph
  logical                              :: restart_mph
  logical                              :: restart_mph_scalar
  logical                              :: restart_mph_heat
  ...
  character(len=1), dimension(0:1,3)   :: cbcsca
  real(rp)        , dimension(0:1,3)   :: bcsca
  character(len=1), dimension(0:1,3)   :: cbcrel
  real(rp)        , dimension(0:1,3)   :: bcrel
  real(rp)                             :: Diff1,Diff2,A_rel,A_m,k_eq,t_relax
  real(rp)                             :: theta,rho_air,rho_vap,mol_mas_air,mol_mas_vap
  real(rp)                             :: cp_air,cv_air,cond_air,cp_vap,cv_vap,cond_vap
  ...
  integer                              :: nthreadsmax
  !
  !  --> from di_acdi.in (always defined in this app)
  !
  real(rp) :: rho1,rho2,mu1,mu2
  character(len=3) :: inipsi
  real(rp),  allocatable, dimension(:) :: xc, yc, zc, r
  character(len=1), dimension(0:1,3) :: cbcpsi
  real(rp)        , dimension(0:1,3) ::  bcpsi
  character(len=1), dimension(0:1,3) :: cbcnor
  real(rp)        , dimension(0:1,3) ::  bcnor
  real(rp) :: sigma,sigma_t
  real(rp) :: gamma_v,eps_int
  logical  :: late_init
...
#endif
  !
#if defined(_TWO_PHASE) && (_DO_POSTPROC) 
  !
  !  --> from post_tw.in (if defined)
  !
  logical :: do_tagging
  integer :: iout0d_ta
  !
  ...
  !
  !
  !  --> from heat_transfer.in (if defined)
  ...
  character(len=1), dimension(0:1,3) :: cbcent
  real(rp)        , dimension(0:1,3) ::  bctmp
  real(rp)        , dimension(0:1,3) ::  bcent
  !
  ...
  subroutine read_input(myid)
    ...
    if( ierr.eq.0 ) then
      ...
      read(iunit,*) restart
      read(iunit,*) restart_sph
      read(iunit,*) restart_mph
      read(iunit,*) restart_mph_scalar
      read(iunit,*) restart_mph_heat
    ...
    close(iunit)
    !  
    ! load scalars input files
    !
    open(newunit=iunit,file='scalar.in',status='old',action='read',iostat=ierr)
    if( ierr.eq.0 ) then 
    read(iunit,*) cbcsca(0,1  ),cbcsca(1,1  ),cbcsca(0,2  ),cbcsca(1,2  ),cbcsca(0,3  ),cbcsca(1,3  )
    read(iunit,*)  bcsca(0,1  ), bcsca(1,1  ), bcsca(0,2  ), bcsca(1,2  ), bcsca(0,3  ), bcsca(1,3  )
    read(iunit,*) cbcrel(0,1  ),cbcrel(1,1  ),cbcrel(0,2  ),cbcrel(1,2  ),cbcrel(0,3  ),cbcrel(1,3  )
    read(iunit,*)  bcrel(0,1  ), bcrel(1,1  ), bcrel(0,2  ), bcrel(1,2  ), bcrel(0,3  ), bcrel(1,3  )
    read(iunit,*) Diff1, Diff2
    read(iunit,*) k_eq, A_m
    read(iunit,*) A_rel
    read(iunit,*) rho_air, rho_vap
    read(iunit,*) mol_mas_air, mol_mas_vap
    read(iunit,*) cp_air, cp_vap
    read(iunit,*) cv_air, cv_vap
    read(iunit,*) cond_air, cond_vap
    read(iunit,*) t_relax
   else
     if(myid.eq.0) print*, 'Error reading the scalar.in input file' 
     if(myid.eq.0) print*, 'Input file missing or incomplete' 
     if(myid.eq.0) print*, 'Aborting...'
     call MPI_FINALIZE(ierr)
     call exit
   ...
   close(iunit)
    ...
    !
    !
    open(newunit=iunit,file='di_acdi.in',status='old',action='read',iostat=ierr)
    if( ierr.eq.0 ) then
      ...
      read(iunit,*) cbcpsi(0,1 ),cbcpsi(1,1 ),cbcpsi(0,2 ),cbcpsi(1,2 ),cbcpsi(0,3 ),cbcpsi(1,3 )
      read(iunit,*) bcpsi(0,1  ),bcpsi(1,1  ),bcpsi(0,2  ),bcpsi(1,2  ),bcpsi(0,3  ),bcpsi(1,3  )
      read(iunit,*) sigma,sigma_t
      read(iunit,*) late_init, i_late_init
      read(iunit,*) gamma_v, eps_int
      read(iunit,*) cbcnor(0,1 ),cbcnor(1,1 ),cbcnor(0,2 ),cbcnor(1,2 ),cbcnor(0,3 ),cbcnor(1,3 )
      read(iunit,*) bcnor(0,1  ),bcnor(1,1  ),bcnor(0,2  ),bcnor(1,2  ),bcnor(0,3  ),bcnor(1,3  )
      read(iunit,*) theta
    else
      if(myid.eq.0) print*, 'Error reading the di_acdi.in input file' 
      if(myid.eq.0) print*, 'Input file missing or incomplete' 
      if(myid.eq.0) print*, 'Aborting...'
      call MPI_FINALIZE(ierr)
      call exit
    ...
    !
#if defined(_TWO_PHASE) && defined(_DO_POSTPROC)
    inquire(file='post_tw.in',exist=exists)
    if(exists) then
      open(newunit=iunit,file='post_tw.in',status='old',action='read',iostat=ierr)
      if( ierr.eq.0 ) then
        read(iunit,*) do_tagging,iout0d_ta
      else
        if(myid.eq.0) print*, 'Error reading the post_tw.in input file' 
        if(myid.eq.0) print*, 'Input file missing or incomplete' 
        if(myid.eq.0) print*, 'Aborting...'
        call MPI_FINALIZE(ierr)
        call exit
    ...
    if( ierr.eq.0 ) then
      ...
      read(iunit,*) cp1,cp2
      read(iunit,*) cv1,cv2
      read(iunit,*) cond1,cond2
      read(iunit,*) h01,h02
      read(iunit,*) cbctmp(0,1 ),cbctmp(1,1 ),cbctmp(0,2 ),cbctmp(1,2 ),cbctmp(0,3 ),cbctmp(1,3 )
      read(iunit,*) cbcent(0,1 ),cbcent(1,1 ),cbcent(0,2 ),cbcent(1,2 ),cbcent(0,3 ),cbcent(1,3 )
      read(iunit,*) bctmp(0,1  ),bctmp(1,1  ),bctmp(0,2  ),bctmp(1,2  ),bctmp(0,3  ),bctmp(1,3  )
      read(iunit,*) bcent(0,1  ),bcent(1,1  ),bcent(0,2  ),bcent(1,2  ),bcent(0,3  ),bcent(1,3  )
#if defined(_BOUSSINESQ)
    ...
    ! Anyway, rho_sp and mu_sp are not employed if (_TWO_PHASE) flag is active
    !
    rho_sp = rho2 
    mu_sp  = mu2
    !
