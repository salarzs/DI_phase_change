...
module mod_mom
  ...
  subroutine momad_xyz_tw_cen(nx,ny,nz,dxi,dyi,dzi,nh_d,nh_u,dzci,dzfi,u,v,w,mu,rho, &
    ...
    real(rp) :: muxm,muxp,muym,muyp,muzm,muzp, &
                rhoxm,rhoxp,rhoyp,rhoym,rhozp,rhozm
    ...
    real(rp) :: divxp, divxm, divyp, divym, divzp, divzm
    ...
    do k=1,nz
      do j=1,ny
        do i=1,nx
          ...
          !
          !
          uuip  = 0.25_rp*( u(ip,j,k)+u(i,j,k) )*( mfx(i+1,j  ,k  )+mfx(i,j  ,k  ) )  
          uuim  = 0.25_rp*( u(im,j,k)+u(i,j,k) )*( mfx(i-1,j  ,k  )+mfx(i,j  ,k  ) )  
          uvjp  = 0.25_rp*( u(i,jp,k)+u(i,j,k) )*( mfy(i+1,j  ,k  )+mfy(i,j  ,k  ) )  
          uvjm  = 0.25_rp*( u(i,jm,k)+u(i,j,k) )*( mfy(i+1,j-1,k  )+mfy(i,j-1,k  ) )  
          uwkp  = 0.25_rp*( u(i,j,kp)+u(i,j,k) )*( mfz(i+1,j  ,k  )+mfz(i,j  ,k  ) )  
          uwkm  = 0.25_rp*( u(i,j,km)+u(i,j,k) )*( mfz(i+1,j  ,k-1)+mfz(i,j  ,k-1) )  
          !
          !
          ...
          !
          !
          uvip  = 0.25*( mfx(i  ,j,k  )+mfx(i  ,j+1,k  ) )*( v(i,j,k)+v(i+1,j,k) )
          uvim  = 0.25*( mfx(i-1,j,k  )+mfx(i-1,j+1,k  ) )*( v(i,j,k)+v(i-1,j,k) )
          vvjp  = 0.25*( mfy(i  ,j,k  )+mfy(i  ,j+1,k  ) )*( v(i,j,k)+v(i,j+1,k) )
          vvjm  = 0.25*( mfy(i  ,j,k  )+mfy(i  ,j-1,k  ) )*( v(i,j,k)+v(i,j-1,k) )
          wvkp  = 0.25*( mfz(i  ,j,k  )+mfz(i  ,j+1,k  ) )*( v(i,j,k+1)+v(i,j,k) )
          wvkm  = 0.25*( mfz(i  ,j,k-1)+mfz(i  ,j+1,k-1) )*( v(i,j,k-1)+v(i,j,k) )
          ...
          !
          !
          uwip  = 0.25*( w(i,j,k)+w(i+1,j,k) )*( mfx(i  ,j  ,k)+mfx(i  ,j  ,k+1) )
          uwim  = 0.25*( w(i,j,k)+w(i-1,j,k) )*( mfx(i-1,j  ,k)+mfx(i-1,j  ,k+1) )
          vwjp  = 0.25*( w(i,j,k)+w(i,j+1,k) )*( mfy(i  ,j  ,k)+mfy(i  ,j  ,k+1) )
          vwjm  = 0.25*( w(i,j,k)+w(i,j-1,k) )*( mfy(i  ,j-1,k)+mfy(i  ,j-1,k+1) )
          wwkp  = 0.25*( w(i,j,k)+w(i,j,k+1) )*( mfz(i  ,j  ,k)+mfz(i  ,j  ,k+1) )
          wwkm  = 0.25*( w(i,j,k)+w(i,j,k-1) )*( mfz(i  ,j  ,k)+mfz(i  ,j  ,k-1) )
          ...
          dwdt(i,j,k) = &
                        ...
                        dzci(k)*((dwdzp+dwdzp)*muzp-(dwdzm+dwdzm)*muzm - (2.d0/3.d0)*(muzp*divzp-muzm*divzm)) )!/rhoz  
