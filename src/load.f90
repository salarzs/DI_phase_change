...
module mod_load
  ...
  !
  subroutine load_scalar(io,filename,time,t_relax,istep,dto)
    !
    implicit none
    !
    character(len=1), intent(in   ) :: io
    ...
    real(rp)        , intent(inout) :: time, dto,t_relax
    !
    integer :: fh
    !
    select case(io)
    case('r')
      ...
      read(88,*) time,t_relax, dto, istep
      close(88)
    case('w')
      if(myid.eq.0) then
        open (88, file=filename)
        write(88,'(3E15.7, 1I9.8)') time,t_relax, dto, istep
        close(88)
      endif
    end select
    !
