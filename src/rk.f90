...
module mod_rk
  ...
  subroutine rk(space_scheme_mom,f_t1,f_t2,nx,ny,nz,dxi,dyi,dzi,nh_d,nh_u,dzci,dzfi,u,v,w, &
                mfx,mfy,mfz,mu,rho,dudtrko,dvdtrko,dwdtrko)
    !
    !
    !inout velocities are onlu velocities but the outputs are the velocity and density product
    !
    ...
    real(rp)        , intent(inout), dimension(1-nh_u:,1-nh_u:,1-nh_u:) :: u,v,w,mfx,mfy,mfz
...
!    case('cen') ! ii order central scheme in divergence form
    call momad_xyz_tw_cen(nx,ny,nz,dxi,dyi,dzi,nh_d,nh_u,dzci,dzfi,u,v,w,mu,rho,mfx,mfy,mfz,dudtrk,dvdtrk,dwdtrk)  
...
!#endif
    ...
    do k=1,nz
      do j=1,ny
        do i=1,nx
          !
          rhox = 0.5*(rho(i,j,k)+rho(i+1,j,k))
          rhoy = 0.5*(rho(i,j,k)+rho(i,j+1,k))
          rhoz = 0.5*(rho(i,j,k)+rho(i,j,k+1))
          !
          u(i,j,k) = u(i,j,k)*rhox + f_t1*dudtrk(i,j,k) + f_t2*dudtrko(i,j,k)
          v(i,j,k) = v(i,j,k)*rhoy + f_t1*dvdtrk(i,j,k) + f_t2*dvdtrko(i,j,k)
          w(i,j,k) = w(i,j,k)*rhoz + f_t1*dwdtrk(i,j,k) + f_t2*dwdtrko(i,j,k)
          !
