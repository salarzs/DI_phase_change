...
module mod_chkdt
  !
  use mpi
  use mod_param     , only: gacc_x,gacc_y,gacc_z,small,cfl_c,cfl_d,pi, &
                            diff1,diff2
  ...
  !
#if defined(_TWO_PHASE)
  subroutine chkdt_tw(nx,ny,nz,dxi,dyi,dzi,nh_d,nh_u,dzci,dzfi,u,v,w,dtmax,t_rel)
    !
...
!
    ...
    if(sigma.eq.0._rp) then
      ...
      dtik = sqrt(4*sigma*pi/(min(rho1,rho2))*dlmini**3)
    endif
...
#endif
    !
if(myid.eq.0) print*, "time-step restric:conv,mom-diff,ene,sca,sur,gr,trel:"&
                       ,dtic**(-1),dtiv**(-1),dtth,dts,dtik**(-1),dtig**(-1),t_rel
    !
    return
