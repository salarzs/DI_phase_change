    ...
    do k=1,nz
      do j=1,ny
        do i=1,nx
          ...
          kp = k+1  
          !
          phix   = 0.5_rp*(psi(ip,j,k)+psi(i,j,k))
          phiy   = 0.5_rp*(psi(i,jp,k)+psi(i,j,k))
          phiz   = 0.5_rp*(psi(i,j,kp)+psi(i,j,k))
          kappax = 0.5_rp*(kappa(ip,j,k)+kappa(i,j,k))
          kappay = 0.5_rp*(kappa(i,jp,k)+kappa(i,j,k))
          kappaz = 0.5_rp*(kappa(i,j,kp)+kappa(i,j,k))
          !
          u(i,j,k) = u(i,j,k) + f_t12*(6.d0*sigma*kappax*phix*(1.d0-phix)*(psi(ip,j,k)-psi(i,j,k)))*dxi
          v(i,j,k) = v(i,j,k) + f_t12*(6.d0*sigma*kappay*phiy*(1.d0-phiy)*(psi(i,jp,k)-psi(i,j,k)))*dyi
          w(i,j,k) = w(i,j,k) + f_t12*(6.d0*sigma*kappaz*phiz*(1.d0-phiz)*(psi(i,j,kp)-psi(i,j,k)))*dzci(k)
          !
          !
