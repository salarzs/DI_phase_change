...
module mod_source
  ...
  use mod_di_acdi   , only: marangoni_he
  !
  implicit none
  !
  private
  ...
  subroutine mar_src(dzfi,nx,ny,nz,nh_d,nh_u,nh_t,f_t12,dxi,dyi,dzi,dzci,delta_arr,nor,kappa,psi,tmp,rho,u,v,w)
    !
   ...
   call marangoni_he((/nx,ny,nz/),(/dxi,dyi,dzi/),1./dzci,1./dzfi,tmp,nh_t,delta_arr,kappa,nor,psi,rho,mar_force)


   do k=1,nz
      do j=1,ny
        do i=1,nx
          u(i,j,k) = u(i,j,k) + f_t12*mar_force(i,j,k,1)
          v(i,j,k) = v(i,j,k) + f_t12*mar_force(i,j,k,2)
          w(i,j,k) = w(i,j,k) + f_t12*mar_force(i,j,k,3)
        enddo
      enddo
    enddo
    !$acc end kernels
    !
    return
  end subroutine mar_src
  !
  !
  subroutine surft_src(nx,ny,nz,nh_d,nh_u,f_t12,dxi,dyi,dzi,dzci,kappa,psi,rho,u,v,w)
    !
    !the input velocities are actually rho.u and the output as well
    !
    use mod_param, only: sigma
    !
    implicit none 
      ...
      do j=1,ny
        do i=1,nx
         ...
         !
          ...
          !
          u(i,j,k) = u(i,j,k) + f_t12*(sigma*kappax*(psi(ip,j,k)-psi(i,j,k)))*dxi
          v(i,j,k) = v(i,j,k) + f_t12*(sigma*kappay*(psi(i,jp,k)-psi(i,j,k)))*dyi
          w(i,j,k) = w(i,j,k) + f_t12*(sigma*kappaz*(psi(i,j,kp)-psi(i,j,k)))*dzci(k)
          !
#else
          psix   = 0.5_rp*(psi(ip,j,k)+psi(i,j,k))
          psiy   = 0.5_rp*(psi(i,jp,k)+psi(i,j,k))
          psiz   = 0.5_rp*(psi(i,j,kp)+psi(i,j,k))
          !Its not clear if the Spurious current reduces with Energy-based CSF
          u(i,j,k) = u(i,j,k) + f_t12*(sigma*kappax*(psi(ip,j,k)-psi(i,j,k)))*dxi!*6.d0*psix*(1.d0-psix)
          v(i,j,k) = v(i,j,k) + f_t12*(sigma*kappay*(psi(i,jp,k)-psi(i,j,k)))*dyi!*6.d0*psiy*(1.d0-psiy)
          w(i,j,k) = w(i,j,k) + f_t12*(sigma*kappaz*(psi(i,j,kp)-psi(i,j,k)))*dzci(k)!*6.d0*psiz*(1.d0-psiz)
#endif
          !
          !
        enddo
  ...
  !
#if defined(_TURB_FORCING)
  subroutine forc_src(nx,ny,nz,nh_d,nh_u,f_t12,dx,dy,dz,zc,rho,u,v,w)
    !
      ...
      do k=1,nz
        do j=1,ny
          do i=1,nx
            ...
            xff = (i+ijk_start_x-0.0_rp)*dx/lx*2._rp*pi
            !
            rhox = (rho(i,j,k)+rho(i+1,j,k))*0.5d0
            rhoy = (rho(i,j,k)+rho(i,j+1,k))*0.5d0
            rhoz = (rho(i,j,k)+rho(i,j,k+1))*0.5d0
            !
            u(i,j,k) = u(i,j,k) + f_t12*( + f0_t*(abc_x*sin(k0_t*zcc) + abc_z*cos(k0_t*ycc)) )*rhox
            v(i,j,k) = v(i,j,k) + f_t12*( + f0_t*(abc_y*sin(k0_t*xcc) + abc_x*cos(k0_t*zcc)) )*rhoy
            w(i,j,k) = w(i,j,k) + f_t12*( + f0_t*(abc_z*sin(k0_t*ycc) + abc_y*cos(k0_t*xcc)) )*rhoz
            !
          enddo
        enddo
      enddo
